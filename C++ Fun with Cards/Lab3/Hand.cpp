...
void Hand::setHandRank() {
	...
	string rank = PokerHandEval(hands);
	if (rank == "Straight Flush") {
		h_rank = HandRanks::straight_flush;
	}
	if (rank == "Straight") {
		h_rank = HandRanks::straight;
	}
	if (rank == "Flush") {
		h_rank = HandRanks::flush;
	}
	if (rank == "Four-of-a-Kind") {
		h_rank = HandRanks::four_kind;
	}
	if (rank == "Three-of-a-Kind") {
		h_rank = HandRanks::three_kind;
	}
	if (rank == "Full House") {
		h_rank = HandRanks::full_house;
	}
	if (rank == "Two Pairs") {
		h_rank = HandRanks::two_pairs;
	}
	if (rank == "One Pair") {
		h_rank = HandRanks::one_pair;
	}
	else h_rank = HandRanks::no_rank;
}
...
void Hand::remove_card(size_t i) {
	if (i >= H.size()) {
		cout << "This card cannot be removed." << endl;
	}
	else {
		H.erase(H.begin() + i);
	}
...
void Hand::push(Card c) { //adds a card to the hand
	H.push_back(c);
